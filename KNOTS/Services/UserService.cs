...
{
    // USER KLASĖ (su [Key] anotacija SQLite)
    ...
    // USERSERVICE KLASĖ (su SQLite ir pataisytomis užklausomis)
    public class UserService
    {
        ...
        public (bool Success, string Message) LoginUser(string username, string password)
        {
            ...
            try
            {
                ...
                if (user != null)
                {
                    ...
                    Console.WriteLine($"Stored hash starts with: {user.PasswordHash.Substring(0, Math.Min(20, user.PasswordHash.Length))}...");
        ...
        public void UpdateUserStatistics(string username, double compatibilityScore, bool wasBestMatch)
        {
            ...
            {
                Console.WriteLine($"❌ User '{username}' not found for statistics update");
            }
        }
