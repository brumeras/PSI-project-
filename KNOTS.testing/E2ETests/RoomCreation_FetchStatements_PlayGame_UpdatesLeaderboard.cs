...
public class RoomCreationFetchStatementsPlayGameUpdatesLeaderboard : EndToEndTestBase
{
    ...
    public void RoomCreationThroughGameFlow_UpdatesLeaderboardAndRanks()
    {
        ...
        CompatibilityService.SaveSwipe(room.RoomCode, "host", "LB-2", true);
        ...
        CompatibilityService.SaveSwipe(room.RoomCode, "guest-two", "LB-2", true);
        ...
        Assert.Equal(new[] { "guest-two", "host", "guest-one" }, leaderboard.Select(u => u.Username).ToArray());
        Assert.Equal(new[] { 100.0, 100.0, 50.0 }, leaderboard.Select(u => u.AverageCompatibilityScore).ToArray());
